...
class IPReputationServices(Visualizer):
    ...
    def _filescan_search(self):
        try:
            ...
            ip = analyzer_report.report.get("ioc_value", "")
    
            filescan_search_report = self.Title(
                self.Base(
                    value="FileScan_Search",
                    link="https://www.filescan.io/api/reputation/ip?ioc_value={ip}",
                    # 'https://www.filescan.io/api/reputation/ip?ioc_value=49.245.55.144'
                    #icon=VisualizableIcon.INFO
                ),
